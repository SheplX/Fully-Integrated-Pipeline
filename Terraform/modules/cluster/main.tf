...
resource "google_container_node_pool" "node" {
  name       = "${lower(var.cluster_name)}-${var.cluster_node.pool_name}-pool"
  location   = var.cluster_location
  cluster    = google_container_cluster.gcp_cluster.name
  node_count = var.cluster_node.node_count

  node_config {
    preemptible  = false
    disk_size_gb = var.cluster_node.disk_size
    machine_type = var.cluster_node.machine_type
    node_image   = var.cluster_node.node_image
